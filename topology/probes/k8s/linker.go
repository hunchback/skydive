...
func (l *ABLinker) AppendNewEdge(edges []*graph.Edge, aNode, bNode *graph.Node) []*graph.Edge {
	id := graph.GenID(string(aNode.ID), string(bNode.ID), "Relationtyp", l.typ)
	m := NewEdgeMetadata(l.manager, l.typ)
	return append(edges, l.graph.CreateEdge(id, aNode, bNode, m, graph.TimeUTC(), ""))
}
